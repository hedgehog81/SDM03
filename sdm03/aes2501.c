...
static int read_data(struct fp_img_dev *dev, unsigned char *data, size_t len)
{
	...
	if (r < 0) {
		...
		fp_err("unexpected short read %d/%d", r, len);
		return -EIO;
	}
	return 0;
}
...
static int dev_init(struct fp_img_dev *dev, unsigned long driver_data)
{
...
#ifndef _WIN32_WCE	
	struct usb_device* usb_dev = usb_device(dev->udev);
	
	usb_set_configuration(dev->udev,usb_dev->config->bConfigurationValue);
#endif
	...
	if (r != 0)
	{
		fp_err("Unable to init the device");
		goto err1;
	}

	dev->priv = fpi_img_new(dev,(3 * MAX_FRAMES * FRAME_SIZE) / 2);
	
	if (!dev->priv)
	{
		fp_err("Unable to to allocate image buffer");
		r = -1;
		goto err1;	
	};
...
{
	int r;
	struct fp_img* img = (struct fp_img*)dev->priv;
	...
	//img = (struct fp_img*)dev->priv;//fpi_img_new((3 * MAX_FRAMES * FRAME_SIZE) / 2);
/*
	imgptr = img->data;
...
*/
	...
	imgptr = img->data + (MAX_FRAMES * FRAME_SIZE);
	...
	flags = FP_IMG_COLORS_INVERTED;
/*	
	height = assemble(img->data, cooked, nstrips, FALSE, &errors_sum);
	height = assemble(img->data, cooked, nstrips, TRUE, &r_errors_sum);
*/
	imgptr = img->data + (MAX_FRAMES * FRAME_SIZE);
	cooked = img->data;

	height = assemble(imgptr, cooked, nstrips, FALSE, &errors_sum);
	height = assemble(imgptr, cooked, nstrips, TRUE, &r_errors_sum);
	
	if (r_errors_sum > errors_sum) {
	    //height = assemble(img->data, cooked, nstrips, FALSE, &errors_sum);
		height = assemble(imgptr, cooked, nstrips, FALSE, &errors_sum);
		flags |= FP_IMG_V_FLIPPED | FP_IMG_H_FLIPPED;
		fp_dbg("normal scan direction");
	} else {
		fp_dbg("reversed scan direction");
	}

	final_size = height * FRAME_WIDTH;

#ifdef _WIN32_WCE
	img->length = final_size;
	*ret = img;
#else
	*ret = fpi_img_new(final_size);
#endif

	(*ret)->flags = flags;
	(*ret)->height = height;
	(*ret)->width = FRAME_WIDTH;

#ifndef _WIN32_WCE
	memcpy((*ret)->data, cooked, final_size);
#endif
...
struct fp_img_driver aes2501_driver = {
	...
	/*Denis*/
	.flags = FP_IMGDRV_SUPPORTS_UNCONDITIONAL_CAPTURE/*0*/,
	.img_height = -1,
	.img_width = 192,
